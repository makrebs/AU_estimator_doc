...
The dataset is \unit[850]{s} long and includes about the same number of input vectors than a real dataset from the blimp.
...
Figure~\ref{fig:result_inputlength} shows the effects of different dataset lengths.
...
As introduced in section~\ref{sec:sim_convergence} there are two measures of dataset length which influence the result in different ways.
The same applies to the confidence calculated from the covariance matrix.
When using more samples per input vector the covariance matrix gets over-confident. (??BLA BLA EXPLAIN FURHTER??)
...
In figure~\ref{fig:result_95pc_confidence} the calculated variance is compared to the measured variance.
The variances are illustrated with circles that represent the region in which the parameters reside with 95\% certainty.
The size of the circles is determined with \eqref{eq:variance_to_95pc}.
...
\caption{Distribution of actuator position estimates. 16 dataset with each 24 input vectors and one sample per input vector.
$\mathbf{+}$ represent the results used to measure the variance.
\textbf{solid}: 95\% confidence interval calculated from measured variance. 
