...
Concretely, we solve a minimization problem with cost function \cite{Seber}

\begin{equation}
S(\theta) = \sum_{j=1}^n | y_j - f_j(x_j, \theta) |
\end{equation}

where $y_j$ are the measurements and $f_j(x_j, \theta)$ are the corresponding model outputs at time step $j$.
\\

In the following, we will state describe the system model and its parameters.
...
The aerodynamic term can be modelled as aerodynamic friction. According to Kundu, the wall shear stress is often expressed in terms of the dimensionless skin friction coefficient $c_f$ as \cite{Kundu2012}
\begin{equation*}
\tau_{wall} = 
...
For the angular acceleration introduced above, we formulate the parameterized system model as
