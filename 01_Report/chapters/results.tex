...
\\
To express the certainty of the inertia tensor estimate, the relative error of the inertia tensor parameters will be used in \cref{fig:err_cmp_sim,fig:err_cmp_real}. ($\left(result-true \right)/norm(true)$)
...
The covariances\footnote{actually just the diagonal entries of the covariance matrix are used to draw the circles} are illustrated with circles that represent the region in which the parameters reside with 95\% certainty.
