...
$\|\mathbf{M}^{aero}(\omega = \unit[0.8]{rad/s})\| \approx \unit[0.2XXXX]{Nm} \Leftrightarrow 
\| \mathbf{M}^{actuation}_{max} \| \approx \unit[15]{Nm}$
). 
Therefore we neglect this term and do not further bother about its direction.
In section~\ref{sub:est_perf} we will show by simulation that it is indeed valid to neglect this term.
...
Every variable in \cref{eq:sys_mod} must be classified as being part of the system state $\mathbf{x}$, the input $\mathbf{u}$, the parameter $\boldsymbol{\theta}$ or as a (constant) known variable\footnote{In this work we only use time invariant variables.
...
\hline
\end{tabular}
\caption{Classification of all variables of the system model equation. All variables that are subject to estimation are introduced as parameter.}
\label{tab:params}
\end{table}
...
XXXXXXXXXXX REPLACE PARAGRAPHS BY A TABLE ?????????????? -> NÖ
...
As an example, we apply the calculation of the motor position variance given the Gibbs-Rodriquez parameters $\boldsymbol{\lambda}$ of the the motor orientation.
